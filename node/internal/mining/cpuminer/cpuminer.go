...
func (m *CPUMiner) solver(ctx context.Context, template *mining.BlockTemplate,
	...
	for {
		...
		//
		// The function will return early with false when the block was not solved
		// for any reason such as the context being cancelled or an unexpected
		// error, so allow it to loop around to potentially try again in that
		// case.
		//
...
func (m *CPUMiner) generateBlocks(ctx context.Context, workerID uint64) {
	...
	speedStats.updateHashes = make(chan uint64, 1)
	m.Lock()
